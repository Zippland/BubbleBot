...
3. **群聊心跳潜水（Alpha）**：bot 每隔 `agents.defaults.group_heartbeat.interval_minutes`（默认 5 分钟）扫描已开启心跳的群聊里自上次扫描以来的新消息，用主模型走一次完整 agent 循环，由模型自己判断"是否该插话"——判断该插话就正常回复，判断不该就调 `pass_heartbeat` 工具静默结束。**开关按群粒度**，需要在群内 `@bot /heartbeat on` 显式开启，状态落在该会话自身的配置里；默认关闭。**"被 @ 必回"的强承诺不变**，心跳只是给未 @ 的群聊消息一个异步追加的回复机会。
